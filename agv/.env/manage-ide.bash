...
#
# @param[in] extensions_tar_gz 扩展包文件路径，默认值为 `${AGV_WORKDIR}/.vscode/extensions.tar.gz`。
# @param[in] target_dir 目标目录路径，默认值为 `${HOME}/.vscode-server`。
#
# @return 返回值为 0 表示成功，非 0 表示失败。
#
# @note
# - 确保 `${AGV_WORKDIR}` 和 `${HOME}` 变量已正确定义。
...
function import-vscode-extensions() {
  local extensions_tar_gz="${AGV_WORKDIR}/.vscode/extensions.tar.gz"
...
function update-gitconfig() {
  ...
  CONFIG_CONTENT=$(cat ${AGV_WORKDIR}/.env/gitconfig)
...
function update-bashrc-config() {
  ...
  CONFIG_CONTENT=$(cat ${AGV_WORKDIR}/.env/bashrc)
  CONFIG_CONTENT=$(echo "$CONFIG_CONTENT" | sed "s|replace_to_script_path|$AGV_WORKDIR|g")
...
function update-ssh-config() {
  ...
  CONFIG_CONTENT=$(cat ${AGV_WORKDIR}/.env/sshconfig)
