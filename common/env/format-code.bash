...
#   search_path: 搜索 `package.xml` 文件的根路径，默认为 `${AI2_WORKDIR}/src`。
#   format_file: `.clang-format` 配置文件的路径，默认为 `${AI2_WORKDIR}/.clang-format`。
#
# 使用示例：
#   format-packages "package1" "package2"
#
# 注意事项：
#   1. 确保 `AI2_WORKDIR` 环境变量已正确设置。
#   2. 确保 `.clang-format` 配置文件存在且格式正确。
#   3. 如果目标文件夹中存在 `COLCON_IGNORE` 文件，则跳过该文件夹的格式化操作。
#   4. 该函数会递归查找 `search_path` 下的所有 `package.xml` 文件。
#
export clang_format_path="${PROJECT_WORKDIR}"/.clang-format
function format-packages() {
  local package_names=("$@")  # 从参数中获取 package_name 列表
  local search_path="${PROJECT_WORKDIR}/src"  # 指定搜索路径
